...
function splitContent{
    for($i=0; $i -lt $global:arrScript.Length; $i++)
    {
        ...
        $isMatch = $line -match "^(SET\s+\@|set\s+\@)(\w+)(\s+\=\s+)(\'\'|0|\'\d+\'|\d+|\-\d|\'[\d\.\s\:]+\'|\'[\s\d\w]+\'|\'.+\')(\s+\-\-\s*|\s+$|\s+\/\*\s*)(.+)"
        
        if(!$isMatch)
        {
            $isMatch = $line -match "^(SET\s+\@|set\s+\@)(\w+)(\s+\=\s+)(\'\'|0|\'\d+\'|\d+|\-\d|\'[\d\.\s\:]+\')"
        }
        else
        {
        ...
        {                           #"^(SET|set\s+\@)      (\w+)(\s+\=\s+)(\'\'|0|\'\d+\'|\d+|\-\d|\'[\d\.\s\:]+\'|\'[\s\d\w]+\')(\s+\-\-\s*|\s+$|\s+\/\*\s*)(.+)"
            $isMatch = $line -match "^(SELECT\s+\@|select\s+\@)(\w+)(\s+\=\s+)(\'\'|0|\'\d+\'|\d+|\-\d|\'[\d\.\s\:]+\'|\'[\s\d\w]+\'|\'.+\')(\s+\-\-\s*|\s+$|\s+\/\*\s*)(.+)"
        }
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if($isMatchComment)
                    {
                        ...
                        $linenew = $linenew -replace "\*\/", ""
                        $linenew = $linenew -replace "\t", ""
                        $linenew = $linenew -replace "\/\*[\s\t]*", ""
                        $linenew = $linenew -replace "[\s\t]*\*\/", ""
                        $linenew = $linenew -replace "[\s\t]{2,}", " "
                    
                        Write-Host "Kommentar2 $linenew"
                        if($global:strPrevComment -ne "")
                        {
                            $global:strPrevComment = $global:strPrevComment + "`r`n" + $linenew
                        }
